...
void main(){
  ...
  float smoke=1.-dot(col,vec3(.21,.71,.07));
  smoke=smoothstep(.18,.95,smoke);
  smoke*=min(time*.1,1.);

  vec3 smokeColor=u_color*smoke;
  smokeColor+=u_color*pow(smoke,2.)*.35;
  O=vec4(smokeColor,smoke*.55);
}`;
...
void main(){gl_Position=position;}`;
  ...
  private targetColor: [number, number, number] = [0.5, 0.5, 0.5];
  ...
  render(now = 0) {
    ...
    this.color[0] += (this.targetColor[0] - this.color[0]) * 0.08;
    this.color[1] += (this.targetColor[1] - this.color[1]) * 0.08;
    this.color[2] += (this.targetColor[2] - this.color[2]) * 0.08;
